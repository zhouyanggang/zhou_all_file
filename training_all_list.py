...
#设定文件的保存路径
root='/home/zhouyanggang/zhou_file/my_python_caffe/'                           #根目录
train_list='/home/zhouyanggang/mnist/train/train.txt'     #训练图片列表
test_list='/home/zhouyanggang/mnist/test/test.txt'        #测试图片列表
train_proto=root+'train_list_mnist.prototxt'     #训练配置文件
test_proto=root+'test_list_mnist.prototxt'       #测试配置文件
solver_proto=root+'solver_list_mnist.prototxt'   #参数文件

#编写一个函数，生成配置文件prototxt
def Lenet(img_list,batch_size,include_acc=False):
    #第一层，数据输入层，以ImageData格式输入
    data, label = L.ImageData(source=img_list, batch_size=batch_size, ntop=2,transform_param=dict(scale= 0.00390625))
    #第二层：卷积层
